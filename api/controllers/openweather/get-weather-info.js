...
module.exports = {
  ...
  description:
    `Pulls 5day forecast from the openweather API and cahces for certain timeperiod.`,
  ...
  fn: async function({ lat, lng, exclude }, exits) {
    ...
    const hasDataExpired = function({ updatedAt }) {
      if (updatedAt) {
        return (
          dateFNS.differenceInMinutes(new Date(), new Date(updatedAt)) > 30
        );
      }
      return true;
    };
    ...
    if (!existingWeatherData) {
      ...
      const weatherData = Object.assign(skyData, {
        daily: Object.assign(skyData.daily, {
          data: _.map(skyData.daily.data, item =>
            _.pick(item, weatherProperties)
          )
        }),
